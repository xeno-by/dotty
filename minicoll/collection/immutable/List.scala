...
sealed abstract class List[+A] extends AbstractSeq[A]
   ...
   *    @inheritdoc
   */
  @inline final def mapConserve[B >: A](f: A => B): List[B] = {
    ...
    def loop(mapped: ListBuffer[B], unchanged: List[A], pending: List[A]): List[B] =
      if (pending.isEmpty) {
        ...
        if (head1 == head0.asInstanceOf[AnyRef])
  ...
  override def takeRight(n: Int): List[A] = {
    ...
    def loop(lead: List[A], lag: List[A]): List[A] = lead match {
      case Nil => lag
      case _ :: tail => loop(tail.asInstanceOf[List[A]], lag.tail)
    }
    loop(drop(n), this)
  }
...
object List extends SeqFactory[List] {
   ...
   */
  class GenericCanBuildFrom[A] extends CanBuildFrom[List, A, List[A]] {
    /** Creates a new builder on request of a collection.
     *  @param from  the collection requesting the builder to be created.
     *  @return the result of invoking the `genericBuilder` method on `from`.
     */
    def apply(from: GenTraversable[_]) = from.genericBuilder[A]
